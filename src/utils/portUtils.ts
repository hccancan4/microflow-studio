  ...
  switch (comp.type) {
    ...
    case 'curved_channel': {
      ...
      return [
        ...
        {
          index: 1,
          localPos: { x: safeRadius * Math.sin(rad), y: safeRadius * (1 - Math.cos(rad)) },
          type: 'output',
          label: 'Çıkış',
          diameter: width,
        },
    ...
    case 't_junction': {
      ...
      return [
        ...
        {
          index: 2,
          localPos: { x: 0, y: -branchWidth * 4 },
          type: 'output',
          label: 'Dal',
          diameter: branchWidth,
        },
      ];
    }
    ...
    case 'y_junction': {
      ...
      return [
        ...
        {
          index: 1,
          localPos: { x: -armLen * Math.sin(rad), y: -armLen * Math.cos(rad) },
          type: 'output',
          label: 'Sol Dal',
          diameter: branchWidth,
        },
    ...
    case 'droplet_generator': {
      ...
      } = p as DropletGeneratorParams;
      return [
        {
          index: 0,
          localPos: { x: -mainChannelWidth * 6, y: 0 },
          type: 'input',
          label: 'Ana Giriş',
          diameter: mainChannelWidth,
        },
        {
          index: 1,
          localPos: { x: mainChannelWidth * 6, y: 0 },
          type: 'output',
          label: 'Çıkış',
          diameter: orificeWidth,
        },
        {
          index: 2,
          localPos: { x: 0, y: -dispersedChannelWidth * 6 },
          type: 'input',
          label: 'Dağılmış',
          diameter: dispersedChannelWidth,
        },
      ];
    }
...
export function checkPortCompatibility(
  ...
  if (fromType === toType) {
    return {
      compatible: false,
      warning: `İki ${fromType === 'output' ? 'çıkış' : 'giriş'} portu birbirine bağlanamaz.`,
    };
  }
