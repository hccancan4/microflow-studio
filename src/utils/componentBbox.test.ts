...
describe('localBbox', () => {
  it('straight_channel: 0..length × ±width/2', () => {
    expect(localBbox(comp('straight_channel', { width: 200, length: 3000 }))).toEqual({
      minX: 0,
      minY: -100,
      maxX: 3000,
      maxY: 100,
  ...
  it('serpentine_mixer: turns*pitch yüksekliği + channelWidth/2 payı', () => {
    ...
    expect(
      localBbox(comp('serpentine_mixer', { turns: 4, pitch: 400, channelWidth: 150 })),
    ).toEqual({ minX: -75, minY: -75, maxX: 1675, maxY: 1675 });
  });

  it('reservoir: 0..width × 0..height', () => {
    expect(localBbox(comp('reservoir', { width: 1500, height: 1500 }))).toEqual({
      minX: 0,
      minY: 0,
      maxX: 1500,
      maxY: 1500,
...
describe('worldBbox', () => {
  ...
  it('rotation=90 → eksen değişimi (yaklaşık)', () => {
    const wb = worldBbox(
      comp('straight_channel', { width: 200, length: 1000 }, { x: 0, y: 0 }, 90),
    );
