...
const PARAM_OPTIONS: Record<ComponentType, ParamOption[]> = {
  ...
  port: [{ key: 'diameter', label: 'Çap', unit: 'μm', min: 50, maxHint: 3000 }],
  droplet_generator: [
    ...
    {
      key: 'dispersedChannelWidth',
      label: 'Dispers kanal genişliği',
      unit: 'μm',
      min: 1,
      maxHint: 2000,
    },
...
export function withOverriddenParam(c: ChipComponent, key: string, value: number): ChipComponent {
  return {
    ...c,
    params: {
      ...(c.params as unknown as Record<string, unknown>),
      [key]: value,
    } as unknown as ChipComponent['params'],
  };
}
