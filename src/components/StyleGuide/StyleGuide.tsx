...
const RADII = [['sm', 2], ['md', 4], ['lg', 8]] as const;
...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
  <section style={{ marginBottom: 'var(--space-8)' }}>
    <h2 style={{
      ...
      color: 'var(--mf-text-dark)', marginBottom: 'var(--space-3)',
      borderBottom: '1px solid var(--mf-border)', paddingBottom: 'var(--space-2)',
    }}>{title}</h2>
    {children}
  </section>
);

const Swatch: React.FC<{ color: string; name: string; note?: string; hex?: string }> = ({ color, name, note, hex }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, minWidth: 110 }}>
    <div style={{
      height: 56, borderRadius: 'var(--radius-md)', background: color,
      border: '1px solid var(--mf-border)', boxShadow: 'var(--elev-1)',
    }} />
    <div style={{ fontSize: 11, color: 'var(--mf-text)', fontWeight: 500 }}>{name}</div>
    {hex && <div className="tabular" style={{ fontSize: 10, color: 'var(--mf-text-dim)', fontFamily: 'var(--font-mono)' }}>{hex}</div>}
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        {/* Başlık */}
        <header style={{ marginBottom: 'var(--space-8)', display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
          <div style={{
            width: 40, height: 40, borderRadius: 'var(--radius-md)',
            background: 'linear-gradient(135deg, var(--mf-dye), var(--mf-dye-dim))',
            color: 'var(--mf-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontWeight: 700, fontSize: 22, letterSpacing: '-0.05em',
          }}>μ</div>
          <div>
            <div style={{ fontSize: 20, fontWeight: 700 }}>MicroFlow Studio — Laminar</div>
            <div style={{ fontSize: 12, color: 'var(--mf-text-dim)' }}>Tasarım Sistemi Vitrini · RAY 0 onay kapısı · <span className="tabular" style={{ fontFamily: 'var(--font-mono)' }}>#{String(tick).padStart(5, '0')}</span></div>
          </div>
        </header>
        ...
        <Section title="Yüzeyler (elevation)">
          <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
            {SURFACES.map((s) => <Swatch key={s.name} color={`var(${s.var})`} name={s.name} note={s.note} />)}
          </div>
        </Section>
        ...
        <Section title="Boya (aksan) + Sinyal renkleri">
          <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
            {ACCENTS.map((s) => <Swatch key={s.name} color={`var(${s.var})`} name={s.name} note={s.note} />)}
            <div style={{ width: 1, background: 'var(--mf-border)', margin: '0 var(--space-2)' }} />
            {SIGNALS.map((s) => <Swatch key={s.name} color={`var(${s.var})`} name={s.name} note={s.note} />)}
          </div>
        </Section>
        ...
        <Section title="Bileşen paleti (canvas ↔ tablo ↔ grafik tek kaynak)">
          <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
            {COMPONENT_PALETTE.map((c) => <Swatch key={c.key} color={c.hex} name={c.label} hex={c.hex} />)}
          </div>
        </Section>
        ...
        <Section title="Tipografi (IBM Plex — self-hosted)">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-6)' }}>
            <div>
              <div style={{ fontSize: 10, color: 'var(--mf-text-dark)', marginBottom: 8 }}>SANS (arayüz)</div>
              {[700, 600, 500, 400].map((w) => (
                <div key={w} style={{ fontFamily: 'var(--font-sans)', fontWeight: w, fontSize: 18, marginBottom: 4 }}>
                  Laminar akış · {w}
                </div>
              ))}
            </div>
            <div>
              <div style={{ fontSize: 10, color: 'var(--mf-text-dark)', marginBottom: 8 }}>MONO (sayılar) · tabular-nums demo</div>
              {[600, 500, 400].map((w) => (
                <div key={w} className="tabular" style={{ fontFamily: 'var(--font-mono)', fontWeight: w, fontSize: 18, marginBottom: 4 }}>
        ...
        <Section title="Bileşenler (buton / input / kart)">
          ...
          <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap', maxWidth: 400 }}>
            <input className="mf-input" placeholder="Metin girişi" defaultValue="200" type="number" />
          </div>
          <div style={{
            marginTop: 'var(--space-4)', padding: 'var(--space-4)', maxWidth: 360,
            background: 'var(--mf-surface)', border: '1px solid var(--mf-border)',
            borderRadius: 'var(--radius-lg)', boxShadow: 'var(--elev-2)',
          }}>
            <div style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--mf-text-dim)', marginBottom: 8 }}>Örnek Kart</div>
            <div className="tabular" style={{ fontSize: 24, fontWeight: 600, fontFamily: 'var(--font-mono)', color: 'var(--mf-dye)' }}>
              {(1.732 + tick / 100000).toFixed(3)} <span style={{ fontSize: 13, color: 'var(--mf-text-dim)' }}>μL/min</span>
            </div>
          </div>
        </Section>
        ...
        <Section title="Spacing (4px tabanlı) + Radius">
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 'var(--space-3)', marginBottom: 'var(--space-4)' }}>
            {SPACING.map(([name, px]) => (
              <div key={name} style={{ textAlign: 'center' }}>
                <div style={{ width: px, height: px, background: 'var(--mf-dye)', borderRadius: 2, margin: '0 auto 4px' }} />
                <div className="tabular" style={{ fontSize: 10, color: 'var(--mf-text-dim)', fontFamily: 'var(--font-mono)' }}>{px}</div>
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
            {RADII.map(([name, px]) => (
              <div key={name} style={{ textAlign: 'center' }}>
                <div style={{ width: 48, height: 48, background: 'var(--mf-elev)', border: '1px solid var(--mf-border-strong)', borderRadius: px }} />
                <div style={{ fontSize: 10, color: 'var(--mf-text-dim)', marginTop: 4 }}>{name} · {px}px</div>
              </div>
            ))}
          </div>
        </Section>
        ...
        <Section title="Grafik teması (recharts — token'lı)">
          <div style={{
            height: 240, background: 'var(--mf-surface)', border: '1px solid var(--mf-border)',
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={SAMPLE_DATA} margin={{ top: 8, right: 16, bottom: 20, left: 8 }}>
                ...
                <XAxis dataKey="x" tick={{ fontSize: 10, fill: 'var(--chart-axis)' }}
                  label={{ value: 'genişlik (μm)', position: 'insideBottom', offset: -8, fontSize: 10, fill: 'var(--chart-axis)' }} />
                ...
                <Line type="monotone" dataKey="flow" stroke="var(--comp-channel)" strokeWidth={2} dot={{ r: 2 }} />
                <Line type="monotone" dataKey="pressure" stroke="var(--comp-junction)" strokeWidth={2} dot={{ r: 2 }} yAxisId={0} hide />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Section>
        ...
        <Section title="Hareket (prefers-reduced-motion saygısı)">
          ...
          <div style={{ fontSize: 11, color: 'var(--mf-text-dark)', marginTop: 'var(--space-2)' }}>
            İşletim sisteminde "hareketi azalt" açıksa animasyon anlık olur (design-system.css media query).
          </div>
        </Section>

        <footer style={{ fontSize: 11, color: 'var(--mf-text-dark)', borderTop: '1px solid var(--mf-border)', paddingTop: 'var(--space-3)' }}>
...
const MotionDemo: React.FC = () => {
  ...
  return (
    <button
      onClick={() => setOn((v) => !v)}
      style={{
        position: 'relative', width: 200, height: 40, borderRadius: 'var(--radius-full)',
        background: 'var(--mf-elev)', border: '1px solid var(--mf-border-strong)', cursor: 'pointer',
      }}
    >
      <span style={{
        position: 'absolute', top: 3, left: on ? 163 : 3, width: 32, height: 32,
        borderRadius: 'var(--radius-full)', background: 'var(--mf-dye)',
        transition: 'left var(--dur-base) var(--ease-fluid)',
      }} />
      <span style={{ position: 'absolute', left: on ? 16 : 44, top: 11, fontSize: 12, color: 'var(--mf-text-dim)' }}>
        {on ? 'açık' : 'kapalı'}
      </span>
    </button>
  );
