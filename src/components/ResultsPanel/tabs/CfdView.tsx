...
export const CfdView: React.FC<{
  ...
  return (
    <div className="grid grid-cols-4 gap-2 p-3 h-full overflow-auto">
      <MetricCard
        label="Maks. Hız"
        value={`${(summary.maxVelocity * 1000).toFixed(3)} mm/s`}
        color="blue"
      />
      <MetricCard label="Debi" value={`${summary.totalFlowRate.toFixed(3)} μL/min`} color="green" />
      <MetricCard
        label="Basınç ∆"
        value={`${(summary.maxPressure - summary.minPressure).toFixed(2)} Pa`}
        color="orange"
      />
      <MetricCard
        label="İterasyon"
        value={`${field.iterations}${field.converged ? ' ✓' : ''}`}
        color={field.converged ? 'green' : 'orange'}
      />

      <MetricCard label="Grid" value={`${field.width}×${field.height}`} color="blue" />
      <MetricCard
        label="Son residual"
        value={(field.residuals?.slice(-1)[0] ?? 0).toExponential(2)}
        color="orange"
      />
      <MetricCard
        label={`${fieldLabel} min`}
        value={`${fmin.toExponential(2)} ${fieldUnit}`}
        color="blue"
      />
      <MetricCard
        label={`${fieldLabel} max`}
        value={`${fmax.toExponential(2)} ${fieldUnit}`}
        color="blue"
      />
      ...
      <div className="col-span-4 bg-mf-bg rounded border border-mf-border p-2">
        <div className="flex items-center justify-between mb-1 text-xs text-mf-text-dim">
          <span>
            {fieldLabel} ({fieldUnit}) — {colormap}
          </span>
          <span className="font-mono">
            {fmin.toExponential(2)} … {fmax.toExponential(2)}
          </span>
        </div>
        <div className="h-3 rounded" style={{ background: colormapCssGradient(colormap, 24) }} />
      </div>

      {/* Residual grafiği */}
      {residualData.length > 1 && (
        <div
          className="col-span-4 bg-mf-bg rounded border border-mf-border p-2"
          style={{ height: 200 }}
        >
          <div className="text-xs text-mf-text-dim mb-1">
            Yakınsama — log₁₀(residual) vs iterasyon
          </div>
...
const ErrorMapPanel: React.FC<{
  ...
  const m = useMemo(() => {
    ...
    let sumExp = 0;
    for (const v of expArr) sumExp += v;
    ...
    for (let i = 0; i < samples.length; i++) {
      ...
      ssr += e * e;
      const d = expArr[i] - meanExp;
      ssTot += d * d;
      if (Math.abs(expArr[i]) > 1e-12) {
        sumPct += Math.abs(e) / Math.abs(expArr[i]);
        usedPct++;
      }
      ...
      <div className="flex items-center justify-between mb-1">
        <div className="text-xs text-mf-text-dim">
          <span
            ...
            style={{ background: dataset.color }}
          />
          Hata haritası — <b>{dataset.name}</b> (sim − deney, |Δ|<sub>max</sub> ={' '}
          {maxAbsErr.toExponential(2)} m/s)
        </div>
        {m && (
...
Δ=${s.err.toExponential(3)} m/s`}</title>
        ...
        <g transform={`translate(${svgW - 120}, ${svgH - 14})`}>
          <text x="0" y="-2" fontSize="9" fill={TOKENS.chartAxis}>
            |hata|: 0
          </text>
          <text x="108" y="-2" fontSize="9" fill={TOKENS.chartAxis} textAnchor="end">
            {maxAbsErr.toExponential(1)}
          </text>
          <rect
            x="0"
            y="0"
