...
export const MetricsCard: React.FC<{
  ...
  const fmtMape = Number.isFinite(metrics.meanAbsPctError)
    ? `${metrics.meanAbsPctError.toFixed(2)}%`
    : '—';
  ...
  return (
    <div className="bg-mf-bg border border-mf-border rounded p-2">
      ...
      <div className="grid grid-cols-4 gap-1 text-xs">
        ...
        <MiniMetric
          label="|Δ|max"
          value={Number.isFinite(metrics.maxAbsError) ? metrics.maxAbsError.toExponential(2) : '—'}
          color="orange"
        />
      </div>
    </div>
...
export const ToggleChip: React.FC<{ label: string; active: boolean; onClick: () => void }> = ({
  label,
  active,
  onClick,
}) => (
...
export const ChartCard: React.FC<{ title: string; children: React.ReactNode }> = ({
  title,
  children,
}) => (
