...
const ScriptEditor: React.FC<ScriptEditorProps> = ({
  ...
  const hasRun =
    !!runStatus &&
    (runStatus.lastActionCount > 0 || runStatus.lastElapsedMs > 0 || hasError || hasOutput);
    ...
    monaco.languages.registerCompletionItemProvider('lua', {
      ...
      provideCompletionItems: () => {
        const suggestions = [
          ...
          // ── chip: metodları ──────────────────────────────────────────
          {
            label: 'chip:add_channel',
            kind: Method,
            insertText:
              'add_channel({x1 = ${1:0}, y1 = ${2:10000}, x2 = ${3:10000}, y2 = ${4:10000}, width = ${5:200}, depth = ${6:50}})',
            insertTextRules: snippet,
            documentation: 'Düz kanal ekle.',
          },
          {
            label: 'chip:add_curved_channel',
            kind: Method,
            insertText:
              'add_curved_channel({cx = ${1:5000}, cy = ${2:5000}, radius = ${3:2000}, start_angle = ${4:0}, end_angle = ${5:180}, width = ${6:200}, depth = ${7:50}})',
            insertTextRules: snippet,
          ...
          },
          {
            ...
            kind: Method,
            insertText:
              'add_port({x = ${1:0}, y = ${2:10000}, type = "${3|inlet,outlet|}", diameter = ${4:500}})',
            insertTextRules: snippet,
            documentation: 'Giriş/çıkış portu ekle.',
          },
          {
            label: 'chip:add_t_junction',
            kind: Method,
            insertText:
              'add_t_junction({x = ${1:5000}, y = ${2:10000}, main_width = ${3:300}, branch_width = ${4:150}, angle = ${5:90}})',
            insertTextRules: snippet,
            documentation: 'T-bağlantı ekle.',
          },
          {
            label: 'chip:add_y_junction',
            kind: Method,
            insertText:
              'add_y_junction({x = ${1:5000}, y = ${2:10000}, main_width = ${3:300}, branch_width = ${4:150}, angle = ${5:60}})',
            insertTextRules: snippet,
            documentation: 'Y-bağlantı ekle.',
          },
          {
            label: 'chip:add_mixer',
            kind: Method,
            insertText:
              'add_mixer({x = ${1:5000}, y = ${2:5000}, channel_width = ${3:200}, turns = ${4:8}, pitch = ${5:600}})',
            insertTextRules: snippet,
            documentation: 'Serpantin mikser ekle.',
          },
          {
            label: 'chip:add_droplet_generator',
            kind: Method,
            insertText:
              'add_droplet_generator({x = ${1:5000}, y = ${2:5000}, dispersed_width = ${3:100}, continuous_width = ${4:200}, orifice_width = ${5:80}})',
            insertTextRules: snippet,
            documentation: 'Damla üreteci (flow-focus / T-jonksiyon) ekle.',
          },
          {
            label: 'chip:add_filter_array',
            kind: Method,
            insertText:
              'add_filter_array({x = ${1:5000}, y = ${2:5000}, width = ${3:2000}, height = ${4:1000}, pillar_diameter = ${5:50}, spacing = ${6:100}})',
            insertTextRules: snippet,
            documentation: 'Pillar array / filtre ekle.',
          },
          {
            label: 'chip:add_expansion',
            kind: Method,
            insertText:
              'add_expansion({x = ${1:5000}, y = ${2:5000}, width_in = ${3:200}, width_out = ${4:600}, length = ${5:1000}})',
            insertTextRules: snippet,
            documentation: 'Kanal genişlemesi/daralması ekle.',
          },
          {
            label: 'chip:add_reservoir',
            kind: Method,
            insertText:
              'add_reservoir({x = ${1:5000}, y = ${2:5000}, radius = ${3:1000}, depth = ${4:200}})',
            insertTextRules: snippet,
          ...
          // ── Lua temel anahtar kelimeler ──────────────────────────────
          {
            ...
            insertTextRules: snippet,
            documentation: 'Sayısal for döngüsü.',
          },
          {
            label: 'while',
            kind: Kw,
      ...
      >
        ...
        >
          <span className="flex items-center gap-1.5">
            ...
            {hasError && <span className="text-mf-orange">● hata</span>}
            {!hasError && hasRun && (
        ...
        {outputOpen && (
          <div className="flex-1 min-h-0 overflow-auto px-3 py-2 font-mono text-xs">
            ...
            {hasOutput && (
              <pre className="text-mf-text whitespace-pre-wrap break-words">{outputLog}</pre>
            )}
