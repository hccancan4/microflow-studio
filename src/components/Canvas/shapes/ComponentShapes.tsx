...
export const ComponentShape: React.FC<ShapeProps> = ({
  ...
  const handleLeave = (e: KonvaEventObject<MouseEvent>) => {
    setHovered(false);
    const c = e.target.getStage()?.container() as HTMLDivElement & { __refreshCursor?: () => void } | undefined;
    if (!c) return;
  ...
  const inner = (() => {
    switch (comp.type) {
      case 'straight_channel':   return <StraightChannel   comp={comp} sw={sw} selected={selected} />;
      case 'curved_channel':     return <CurvedChannel     comp={comp} sw={sw} selected={selected} />;
      case 't_junction':         return <TJunction         comp={comp} sw={sw} selected={selected} />;
      case 'y_junction':         return <YJunction         comp={comp} sw={sw} selected={selected} />;
      case 'serpentine_mixer':   return <SerpentineMixer   comp={comp} sw={sw} selected={selected} />;
      case 'port':               return <PortShape         comp={comp} sw={sw} selected={selected} zoom={zoom} />;
      case 'droplet_generator':  return <DropletGenerator  comp={comp} sw={sw} selected={selected} />;
      case 'filter_array':       return <FilterArray       comp={comp} sw={sw} selected={selected} />;
      case 'expansion':          return <ExpansionShape    comp={comp} sw={sw} selected={selected} />;
      case 'reservoir':          return <ReservoirShape    comp={comp} sw={sw} selected={selected} />;
      default:                   return <FallbackShape     comp={comp} sw={sw} />;
    }
  })();
...
interface BaseProps { comp: ChipComponent; sw: number; selected: boolean; }
...
const YJunction: React.FC<BaseProps> = ({ comp, sw, selected }) => {
  ...
  return (
    <Group>
      {/* Ana kol (aşağı) */}
      <Rect x={-mainWidth / 2} y={0} width={mainWidth} height={armLen} fill={c.fill} stroke={c.stroke} strokeWidth={sw} />
      {/* Sol kol */}
      <Group rotation={-branchAngle}>
        <Rect x={-branchWidth / 2} y={-armLen} width={branchWidth} height={armLen} fill={c.fill} stroke={c.stroke} strokeWidth={sw} />
      </Group>
      {/* Sağ kol */}
      <Group rotation={branchAngle}>
        <Rect x={-branchWidth / 2} y={-armLen} width={branchWidth} height={armLen} fill={c.fill} stroke={c.stroke} strokeWidth={sw} />
  ...
  for (let i = 0; i <= turns; i++) {
    ...
    lines.push(
      <Line
        ...
        stroke={c.stroke} strokeWidth={channelWidth} lineCap="round"
        opacity={0.4}
      />
    );
    lines.push(
      <Line
        key={`hb${i}`}
        points={isEven ? [0, y, totalW, y] : [totalW, y, 0, y]}
        stroke={c.stroke} strokeWidth={sw * 2} lineCap="round"
      />
    );
    ...
    if (i < turns) {
      ...
      lines.push(
        <Line
          ...
          stroke={c.stroke} strokeWidth={channelWidth} opacity={0.4}
        />,
        <Line
          key={`vb${i}`}
          points={[x, y, x, y + pitch]}
          stroke={c.stroke} strokeWidth={sw * 2}
        />
      );
    }
  }
...
interface PortProps extends BaseProps { zoom?: number }
const PortShape: React.FC<PortProps> = ({ comp, sw, selected }) => {
  const { diameter = 500, portType = 'inlet' } = comp.params as PortParams;
  const r = diameter / 2;
  const c = selected ? COLORS.selected : (portType === 'inlet' ? COLORS.port_inlet : COLORS.port_outlet);
...
const DropletGenerator: React.FC<BaseProps> = ({ comp, sw, selected }) => {
  const { orificeWidth = 100, mainChannelWidth = 300, dispersedChannelWidth = 200 } = comp.params as DropletGeneratorParams;
  ...
  return (
    <Group>
      {/* Ana kanal (yatay) */}
      <Rect x={-armLen} y={-mainChannelWidth / 2} width={armLen * 2} height={mainChannelWidth}
        fill={c.fill} stroke={c.stroke} strokeWidth={sw} />
      {/* Dağılmış kanal (dikey) */}
      <Rect x={-dispersedChannelWidth / 2} y={-dispLen} width={dispersedChannelWidth} height={dispLen}
        fill={c.fill} stroke={c.stroke} strokeWidth={sw} />
      {/* Orifis çizgisi */}
      <Rect x={-orificeWidth / 2} y={-mainChannelWidth / 2} width={orificeWidth} height={mainChannelWidth}
...
const FilterArray: React.FC<BaseProps> = ({ comp, sw, selected }) => {
  const { pillarDiameter = 50, spacing = 100, rows = 5, columns = 10 } = comp.params as FilterArrayParams;
  ...
  return (
    <Group>
      <Rect x={0} y={0} width={totalW} height={totalH} fill={c.fill} stroke={c.stroke} strokeWidth={sw} />
      {pillars}
    </Group>
  );
};
  ...
  return (
    <Group>
      <Rect x={0} y={0} width={width} height={height}
        fill={c.fill} stroke={c.stroke} strokeWidth={sw} cornerRadius={width * 0.05} />
      {/* İç dolgu çizgisi */}
      <Rect x={width * 0.1} y={height * 0.1} width={width * 0.8} height={height * 0.8}
        fill="rgba(120,144,156,0.05)" stroke={c.stroke} strokeWidth={sw * 0.5}
        opacity={0.5} cornerRadius={width * 0.03} />
    </Group>
  );
};
