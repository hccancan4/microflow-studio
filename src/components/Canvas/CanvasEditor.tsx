...
import { FiTrash2, FiCopy, FiClipboard, FiRotateCw, FiRotateCcw } from 'react-icons/fi';
...
const CanvasEditor: React.FC<CanvasEditorProps> = ({ width, height }) => {
    ...
    (_e?: Konva.KonvaEventObject<MouseEvent>) => {
      ...
      if (isRubberBanding.current && rubberBand) {
        ...
        if (maxX - minX >= minWorld || maxY - minY >= minWorld) {
          ...
          const inBand = components.filter((c) => bboxesIntersect(worldBbox(c), band));
  ...
  const handleCompContextMenu = useCallback(
  ...
  const handleCompDragEnd = useCallback(
    (e: Konva.KonvaEventObject<DragEvent>, id: string) => {
      ...
      if (selectedIds.length > 1 && selectedIds.includes(id)) {
        ...
        moveComponents(selectedIds.map((sid) => ({ id: sid, dx, dy })));
  ...
  return (
    ...
    >
      ...
      <div className="flex" style={{ height: RULER_SIZE }}>
        ...
        >
          <div className="w-full h-full flex items-center justify-center text-mf-text-dark text-xs font-mono">
            μ
          </div>
        </div>
        {/* Yatay cetvel */}
        ...
        >
          ...
          <div className="absolute bottom-2 right-2 text-2xs text-mf-text-dim bg-mf-surface/85 px-2 py-1 rounded-sm border border-mf-border font-mono tabular tracking-tight pointer-events-none">
            <span className="tabular">
              {mouseUm.x.toFixed(canvas.zoom < 0.5 ? 0 : canvas.zoom < 2 ? 1 : 2)},{' '}
              {mouseUm.y.toFixed(canvas.zoom < 0.5 ? 0 : canvas.zoom < 2 ? 1 : 2)} μm
            </span>
