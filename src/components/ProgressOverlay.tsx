...
const ProgressOverlay: React.FC = () => {
  ...
  const eta = isRunning && progress > 4 && progress < 99
    ...
    <div className="fixed bottom-10 right-4 z-[9998] w-80 pointer-events-none select-none">
      <div className={clsx(
        ...
        {residuals.length > 1 && (
          <div className="mt-1.5">
            <div className="flex items-center justify-between text-[10px] text-mf-text-dark mb-0.5">
              ...
              <span className="font-mono tabular">{residuals[residuals.length - 1].toExponential(1)}</span>
            </div>
        ...
        <div className="mt-1.5 flex items-center gap-2 text-[11px] text-mf-text-dim leading-snug">
          ...
          {eta != null && (
            <span className="font-mono text-mf-text-dark tabular flex-shrink-0">~{formatEta(eta)}</span>
          )}
        </div>
      </div>
    </div>
...
const Sparkline: React.FC<{ values: number[] }> = ({ values }) => {
  ...
  if (!Number.isFinite(min) || !Number.isFinite(max)) { min = -12; max = 0; }
  ...
  return (
    <svg width="100%" height={H} viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none"
         style={{ display: 'block' }}>
      <polyline points={pts} fill="none" stroke={CHART_SERIES.residual} strokeWidth={1.5}
